...
class Game:
    ...
    def main(self):
        while True:
            ...
            if not self.scene_manager.get_scene_data(self.scene_manager.IS_LAST_SCENE):
                ...
                if self.is_user_meeting_requirements(choice_nr):
    ...
    def is_user_meeting_requirements(self, choice_nr):
        if isinstance(choice_nr, int):
            choice_nr -= 1
            next_scene = self.scene_manager.get_choices_data(choice_nr, self.scene_manager.GO_TO_SCENE)
            requirements = self.scene_manager.get_scene_data(self.scene_manager.REQUIREMENTS_FLAGS, next_scene)
            meet_req = 0
            if requirements is None:
                return True
            for req in requirements:
                if req in self.flag_manager.flags:
                    meet_req += 1
            if meet_req == len(requirements):
                return True
            else:
                return False
